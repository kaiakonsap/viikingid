...
function get_boundingBox(x, y, baseline, lineheight, linewidth, weight, color, opacity, rotation) {
    ...
    var w = parseInt(linewidth), b = parseInt(baseline), h = parseInt(lineheight);
...
function get_strokeText(string, x, y, size, weight, width, space, font, color, opacity, rotation, id) {
    ...
    space = typeof(space) != 'undefined' ? space : 100;
    font = typeof(font) != 'undefined' ? font : "sans-serif";
    ...
    var spc = Math.max(Math.min(space, 1000), 10) / 100, mx = ((mag * 16 * faw) * spc) - (mag * 16 * faw), lw = (fac * mag);
    x = 0;
    y = size;
...
function get_textWidth(string, size, width, space, font) {
    ...
    string = typeof(string) != 'undefined' ? string : ' ';
    font = typeof(font) != 'undefined' ? font : "sans-serif";
    var total = 0, len = string.length, mg = size / 25.0, fw = Math.max(Math.min(width, 400), 10) / 100, sp = Math.max(Math.min(space, 1000), 10) / 100, m = ((mg * 16 * fw) * sp) - (mg * 16 * fw);
    for (var i = 0; i < len; i++) {
...
function get_widthText(string, width, size, fontwidth, space, font) {
    ...
    width = typeof(width) != 'undefined' ? width : 100;
    font = typeof(font) != 'undefined' ? font : "sans-serif";
    var cur = 0, total = 0, len = string.length, mg = size / 25.0, fw = Math.max(Math.min(fontwidth, 400), 10) / 100, sp = Math.max(Math.min(space, 1000), 10) / 100, m = ((mg * 16 * fw) * sp) - (mg * 16 * fw);
    for (var i = 0; i < len; i++) {
        var c = strokeFont[font][string.charAt(i)];
        if (c) {
            cur = ((c.w * fw) * mg) + m;
